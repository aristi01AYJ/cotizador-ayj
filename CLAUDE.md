...
- **⚠️ Depende de una configuración de SharePoint que no se puede verificar desde este repo:** `crearLinkAnonimoParaOferta()` llama a Graph `POST /drive/items/{id}/createLink` con `{type:'view',scope:'anonymous'}` — esto SOLO funciona si el sitio de SharePoint (Comercial) tiene habilitado "Cualquier persona con el enlace" en su configuración de uso compartido. Si el tenant lo tiene restringido (política común en organizaciones), Graph responde 403 y el mensaje de error se muestra tal cual al usuario, pidiéndole que hable con su administrador de M365 — no hay forma de saber desde el código si está habilitado hasta que alguien lo pruebe con una cuenta real.
- Botón **"📤 Link para el cliente"** en la barra de la ventana del PDF (junto a "🔗 Ver en SharePoint", solo si `d.linkSharePoint` existe) y en el Historial (ícono ✈️ junto al de PDF, `generarLinkClienteIdx(id)`) — ambos llaman a `crearLinkAnonimoParaOferta(numOferta, cliente, idSynergy)` y muestran el link en un `prompt()` listo para copiar y enviar por correo/WhatsApp.
- **El cliente, al abrir ese link, NO tiene `window.opener`** (no vino de `window.open()` de la app) — el script inyectado lo detecta (`hayOpener`) y en ese caso: oculta "📤 Link para el cliente" y "💾 Guardar firma" (ninguno de los 2 puede funcionar sin sesión de AYJ para escribir a SharePoint) y muestra en su lugar una nota fija: *"Después de firmar, use '⬇ Guardar como PDF' arriba y envíe ese archivo por correo o WhatsApp a AYJ Maquinaria"*. El cliente externo firma en el canvas y usa el botón de imprimir que YA existía — el PDF resultante se lo queda él y nos lo reenvía manualmente; no hay forma de que la firma vuelva sola a SharePoint sin que un usuario de AYJ (con sesión MSAL) la suba, porque este cotizador no tiene backend/credenciales de servicio para escribir sin login.
- Proceso recomendado end-to-end: (1) el vendedor genera la oferta como siempre; (2) clic en "📤 Link para el cliente" y se lo envía; (3) el cliente abre el link (sin necesitar cuenta de Microsoft), firma en el canvas, y guarda/envía el PDF; (4) si se quiere que el registro oficial en SharePoint también quede con la firma, el vendedor reabre el link él mismo (ahí si tiene `window.opener` porque lo abrió desde el Historial) y usa "💾 Guardar firma", o simplemente adjunta el PDF que le devolvió el cliente en otro lugar (fuera del alcance actual de este cotizador).

### Fixes recientes
- **16-sep-2026 — Link de la oferta para que un cliente externo la firme.** Ver sección arriba — depende de que "Cualquier persona con el enlace" esté habilitado en SharePoint (no verificable desde aquí). Aplicado también en V2.
