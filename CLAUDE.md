...
### ⚠️ Requiere columnas nuevas en SharePoint (lista Cotizaciones): `FechaCierre` y `PosibleCierre`
Feature "Posible cierre + Seguimiento" (abajo) necesita 2 columnas en la lista **Cotizaciones** de SharePoint:
- **`FechaCierre`** — tipo **Fecha y hora** (formato "Solo fecha" si SharePoint lo ofrece). Es el dato que de verdad se usa para calcular la Fecha de Seguimiento (`FechaCierre − 5 días`).
- **`PosibleCierre`** — tipo **Una línea de texto**. Guarda solo cuál opción eligió el vendedor ("30 días", "60 días", "90 días" o "Personalizada") como registro histórico — NUNCA se usa para restaurar el selector al editar una oferta (eso siempre usa la fecha exacta guardada, para no recalcular "hoy+30" y correr la fecha real por accidente). Es puramente informativo — se ve junto a la fecha de cierre en la tarjeta de Seguimiento, ej. "Cierre: 24 de sept de 2026 (60 días)".

Nombres exactos, sin tildes/espacios, a propósito — así el nombre interno de SharePoint coincide con el nombre visible y no hay riesgo del problema de codificación `_x00XX_` que ya afectó a Tipología/Ubicación.

**Leer** sin estas columnas no falla (llegan `undefined`, se muestran vacías). **Guardar SÍ fallaba** — bug real reportado por el usuario 17-sep-2026: Graph rechaza el POST completo con `400 invalidRequest: "Field 'X' is not recognized"` por CADA columna que falte, bloqueando el guardado de la oferta ENTERA. Ya corregido — ver `guardarEnSP()` en "Fixes recientes": reintenta en bucle quitando cada campo que Graph rechace (soporta más de una columna faltante a la vez) y avisa con un toast cuáles columnas faltan, para que nunca se pierda una cotización por campos opcionales sin crear.
...
- Se guardan **`FechaCierre`** y **`PosibleCierre`** en SharePoint (`guardarCotizacion()`, vía `obtenerPosibleCierreLabel()`). La **Fecha de Seguimiento nunca se guarda aparte** — siempre se calcula como `FechaCierre − 5 días` (`calcularFechaSeguimiento()`), para no tener 2 fuentes de verdad que se puedan desincronizar si alguien edita una fecha a mano en SharePoint. `PosibleCierre` es puro registro histórico (qué opción se eligió) — no participa en ningún cálculo.
...
### Feature: editar "Posible Cierre" directo desde el Historial (sin crear revisión)
- El Historial tiene una nueva columna **"Seguimiento"** que muestra la Fecha de Seguimiento de cada oferta (o "— Definir" en gris/punteado si no tiene fecha de cierre asignada) — clic en cualquiera de las dos abre `abrirModalCierre(itemId, fechaCierreISO)`: el mismo selector 30/60/90/fecha personalizada de "Nueva oferta", en un modal.
- **A propósito NO reutiliza el flujo "Editar oferta"** (que reabre el carrito y guarda como revisión `-Rn` nueva) — `aplicarCierre()` hace un **PATCH directo** sobre el mismo registro de SharePoint (mismo patrón que `aplicarEstado()`/`editarProbabilidadEstrellas()`). Pensado sobre todo para llenar retroactivamente el Posible Cierre de ofertas viejas (creadas antes de que existiera este campo) sin generar una revisión por cada una.
- Si el registro ya tenía `FechaCierre`, el modal la muestra siempre como fecha personalizada con el valor exacto guardado (mismo criterio que `editarOferta()` — nunca recalcula "hoy+30" al abrir el modal). Elegir "— Sin definir —" y guardar limpia `FechaCierre`/`PosibleCierre` (`null`/`''`).
- Mismo resguardo de columnas faltantes que `guardarEnSP()`: si `aplicarCierre()` recibe `"Field 'X' is not recognized"`, reintenta sin ese campo en vez de fallar.
- En la celda, "— Definir"/la fecha vencida (roja, si la oferta sigue activa y ya pasó la Fecha de Seguimiento) usa el mismo criterio visual que la pantalla de Seguimiento.
...
### Fixes recientes
- **17-sep-2026 — Bug real: FechaCierre nunca se veía en Historial/Seguimiento (parseo de fecha roto).** `cargarHistorial()` leía `f.FechaCierre` asumiendo que Graph devuelve solo la fecha ("2026-10-17") y le concatenaba `'T00:00:00'` a mano — pero Graph devuelve un campo "Fecha y hora" como ISO **completo** ("2026-10-17T00:00:00Z"), así que quedaba `"...T00:00:00ZT00:00:00"`, una fecha inválida, y la oferta se veía exactamente como si nunca se hubiera guardado (mismo síntoma reportado por el usuario: "hago la selección pero no se actualiza"). Nueva función `parsearFechaSharePoint()` soporta ambos formatos sin duplicar el sufijo — se usa en `cargarHistorial()`. De paso, el guardado (`guardarCotizacion()` y `aplicarCierre()`) ahora manda `FechaCierre` como ISO completo (`toISOString()`) en vez de solo la fecha, el formato estándar que Graph espera para un campo DateTime. También: los toasts de error ahora duran 6s en vez de 3s (más difíciles de perderse), y `aplicarCierre()` deja rastro en consola (`console.log`/`console.error`) del payload enviado y la respuesta de Graph, para diagnosticar más rápido si esto vuelve a pasar. Aplicado también en V2.
- **17-sep-2026 — Editar "Posible Cierre" directo desde el Historial (columna Seguimiento + modal, sin crear revisión).** A pedido del usuario, para poder llenar retroactivamente ofertas viejas sin generar una revisión -Rn por cada una. Ver sección arriba. Aplicado también en V2.
- **17-sep-2026 — Columna `PosibleCierre` (registro histórico de qué opción 30/60/90/Personalizada se eligió).** A pedido del usuario, para tener trazabilidad además de la fecha calculada. Ver sección de FechaCierre y "Feature: Posible Cierre" arriba. Aplicado también en V2.
- **17-sep-2026 — El fix anterior solo cubría 1 columna faltante; el usuario reportó que le seguía fallando.** `guardarEnSP()` reintentaba una sola vez — si faltaba más de una columna en SharePoint, el segundo intento (sin la primera) volvía a fallar por la segunda, y el error se veía igual de bloqueante. Ahora reintenta en bucle (hasta 15 veces), quitando un campo a la vez por cada `"Field 'X' is not recognized"` que Graph rechace, hasta que el resto se guarde — y el toast final lista TODAS las columnas que faltan, no solo la primera. Aplicado también en V2.
...
- **16-sep-2026 — Botón "Descargar para enviar por correo" (fallback sin SharePoint).** Confirmado con el usuario real que el sitio Comercial tiene el uso compartido desactivado — el link anónimo falla siempre. Se agregó una alternativa que no depende de esa configuración. Ver sección arriba. Aplicado también en V2.
- **16-sep-2026 — Link de la oferta para que un cliente externo la firme.** Ver sección arriba — depende de que "Cualquier persona con el enlace" esté habilitado en SharePoint (confirmado desactivado por ahora). Aplicado también en V2.
- **16-sep-2026 — Firma digital del cliente en el PDF (canvas dibujable).** Ver sección arriba. Aplicado también en V2.
- **15-sep-2026 — Posible Cierre (30/60/90) + Fecha de Seguimiento automática + pantalla de Seguimiento por usuario.** Ver 2 secciones arriba — requiere crear la columna `FechaCierre` en SharePoint (ver aviso arriba). Aplicado también en V2.
- **15-sep-2026 — Agrupar revisiones (-R2, -R3...) en las estadísticas ("Solo vigentes").** Ver sección arriba. Aplicado también en V2.
- **12-sep-2026 — Asignar/reasignar Gerente de Cuenta a otra persona (no solo a uno mismo).** Ver sección de Gerente de Cuenta arriba. Aplicado también en V2.
- **12-sep-2026 — Ofertas del cliente + estadísticas/gráfico en su ficha.** Ver sección arriba. Aplicado también en V2.
- **12-sep-2026 — Gerente de Cuenta en Clientes, gráfico Ofertas x Mes, overlay de carga bloqueante.** Ver 3 secciones arriba. Aplicado también en V2.
...
- **12-sep-2026 — Cotizaciones duplicadas al re-guardar + Acciones mejoradas.** Ver sección arriba. Aplicado también en V2.
- **10-sep-2026 — Falso "ÚLTIMA UNIDAD" en ítems de stock ilimitado.** Ver sección arriba. Aplicado también en V2.
- **31-ago-2026 — Consecutivo de cotización pegado.** `generarNumOferta()` usaba `graphGet()` sin paginar; se cambió a `graphGetAll()`. Mismo fix aplicado también en V2.
